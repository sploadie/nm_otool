...
/*   Updated: 2016/09/02 15:26:45 by tgauvrit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		get_sections(t_section_num *secs, uint32_t i,
...
t_section_num	*section_numbers(struct load_command *lc, uint32_t ncmds)
{
	...
	while (ncmds-- > 0)
	{
		if (lc->cmd == LC_SEGMENT)
			get_sections(secs, ((struct segment_command*)lc)->nsects,
				(void*)lc + sizeof(struct segment_command),
				sizeof(struct section));
		if (lc->cmd == LC_SEGMENT_64)
			get_sections(secs, ((struct segment_command_64*)lc)->nsects,
